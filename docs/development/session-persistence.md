...
- 摘要正文不会重复展开到系统提示里
- 摘要正文会继续通过 compact 逻辑以单独的 synthetic context message 注入一次
- 这样可以避免“重启恢复后摘要被注入两次”的 token 膨胀问题
...
- 如果崩溃发生在追加下一条 JSONL 的中途，导致最后只落下半行 JSON：
  - 只允许忽略最后一条不完整尾片段
  - 更早的完整行仍然必须严格通过解析
  - 修复完成后会把当前 session 文件重写成干净 JSONL

这样能保证 resume 后的历史始终是可重放的。

## 并发保护

为了避免两个后端实例同时写同一个 `workspace/sessions/`，当前实现会在：

- `workspace/sessions/.sa-session.lock`

上持有一个独占文件锁。

效果：

- 同一时刻只能有一个活跃后端拥有该工作区的持久化 session
- 第二个后端实例启动到 `SessionStore::new()` 时会直接失败
- 这样可以避免 `.current` 与活跃 JSONL 被多个进程交叉改写
...
- 当前设计明确要求：同一个 workspace 只运行一个持久化主后端实例
